...
# make data.frame for QLD & other state's data
df_qld <- data.frame(ausdata$Date, ausdata$QLD, c(0,diff(ausdata$QLD))) # df(Date, Total cases, New cases each day) New cases padded with 0 in first row to make column same length for plotting
colnames(df_qld) <- c("Date", "QLD", "New")
df_nsw <- data.frame(ausdata$Date, ausdata$NSW, c(0,diff(ausdata$NSW))) # df(Date, Total cases, New cases each day) New cases padded with 0 in first row to make column same length for plotting
colnames(df_nsw) <- c("Date", "NSW", "New")
df_vic <- data.frame(ausdata$Date, ausdata$VIC, c(0,diff(ausdata$VIC))) # df(Date, Total cases, New cases each day) New cases padded with 0 in first row to make column same length for plotting
colnames(df_vic) <- c("Date", "VIC", "New")

# Generate plot QI chart using last 30 days data
...
popln_data <- na.omit(owid[owid$location==c("Australia","United States","United Kingdom","South Africa","Italy","Brazil","Cuba","Colombia", "China"),c("date","location","total_cases_per_million","total_deaths_per_million")])
...
popln_cases_plot <- ggplot(popln_data, aes(x=date)) +
  ...
  geom_line(aes(y = total_cases_per_million, group = location, color = location), data = subset(popln_data, location == "China"), size = 1.2) +
...
popln_deaths_plot <- ggplot(popln_data, aes(x=date)) +
  ...
  geom_line(aes(y = total_deaths_per_million, group = location, color = location), data = subset(popln_data, location == "China"), size = 1.2) +
